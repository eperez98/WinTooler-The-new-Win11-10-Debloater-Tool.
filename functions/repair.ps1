...
function Remove-AllRestorePoints {
    try {
        # vssadmin: delete all shadow copies on C:
        $vss = & vssadmin delete shadows /for=C: /all /quiet 2>&1 | Out-String
        Write-WTLog "vssadmin delete shadows: $($vss.Trim())"

        # WMI: remove any SystemRestore shadow copies that vssadmin might miss
        $rps = Get-WmiObject -Class Win32_ShadowCopy -ErrorAction SilentlyContinue |
               Where-Object { $_.VolumeName -like "*C:*" -or $_.SetID -ne "" }
        $count = 0
        if ($rps) {
            foreach ($rp in $rps) {
                $rp.Delete() | Out-Null
        ...
        # Disable then re-enable to flush the restore-point registry list
        Disable-ComputerRestore -Drive "C:\" -ErrorAction SilentlyContinue
        Enable-ComputerRestore  -Drive "C:\" -ErrorAction SilentlyContinue

        Write-WTLog "All restore points deleted."
        return "All system restore points deleted successfully."
    } catch {
        Write-WTLog "Remove-AllRestorePoints error: $_" "ERROR"
        return "Error: $_"
    }
}
